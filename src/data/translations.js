/**
 * translations.js — Bilingual content (ID / EN)
 * Used by i18n.js to switch all visible text
 */

export const translations = {
  id: {
    // Preloader
    preloader_text: 'Robby Hidayat',

    // Hero
    hero_greeting: 'Halo, saya',
    hero_name: 'Robby Hidayat',
    hero_tagline: 'Software Developer yang membangun sistem dengan bebas & cermat.',
    hero_cta_projects: 'Lihat Proyek',
    hero_cta_contact: 'Hubungi Saya',
    hero_role: 'Software Developer',
    hero_scroll: 'Scroll ke bawah',

    // About
    about_label: 'TENTANG SAYA',
    about_title: 'Tentang Saya',
    about_p1: 'Saya Robby Hidayat, lulusan S1 <strong>Ilmu Komputer Universitas Lampung</strong> dengan IPK <strong>3.74/4.00</strong>. Saat ini bekerja sebagai <strong>Staff Programmer</strong> di PT. Lautan Teduh Interniaga, Divisi IT.',
    about_p2: 'Berpengalaman membangun aplikasi web menggunakan <strong>Laravel, React, PHP, MySQL</strong>, dan <strong>JavaScript</strong>. Terbiasa bekerja dari analisis kebutuhan hingga deployment.',
    about_p3: 'Di luar coding, saya pernah memimpin <strong>HIMAKOM Universitas Lampung</strong> sebagai Ketua Umum dengan 142+ anggota aktif — pengalaman yang menempa kemampuan kepemimpinan, komunikasi, dan manajemen proyek saya.',

    // Skills
    skills_label: 'KEAHLIAN',
    skills_title: 'Yang Saya Kuasai',
    skills_languages: 'Bahasa Pemrograman',
    skills_frameworks: 'Framework & Tools',
    skills_soft: 'Soft Skills',

    // Soft skill items
    soft_leadership: 'Kepemimpinan',
    soft_problem: 'Problem Solving',
    soft_project: 'Manajemen Proyek',
    soft_communication: 'Komunikasi',
    soft_teaching: 'Mengajar',

    // Projects
    projects_label: 'PROYEK',
    projects_title: 'Proyek',

    // Certifications
    certs_label: 'SERTIFIKASI',
    certs_title: 'Sertifikasi & Pelatihan',

    // Experience & Organization
    exp_label: 'PENGALAMAN',
    exp_title: 'Pengalaman',
    exp_col_title: 'Pengalaman Kerja',
    org_col_title: 'Organisasi',

    // Boarding Pass
    bp_airline: 'BYRON AIRWAYS',
    bp_flight: 'DEV-2026',
    bp_gate: 'SKY-01',
    bp_seat: '01A',
    bp_class: 'FIRST CLASS',
    bp_passenger: 'ROBBY HIDAYAT',
    bp_destination: 'FULLSTACK & AI',
    bp_status: 'BOARDED',

    // Contact
    contact_label: 'KONTAK',
    contact_title: 'Mari Terhubung',
    contact_name: 'Nama',
    contact_email: 'Email',
    contact_message: 'Pesan',
    contact_name_placeholder: 'Nama lengkap',
    contact_email_placeholder: 'email@contoh.com',
    contact_message_placeholder: 'Tulis pesan Anda di sini...',
    contact_submit: 'Kirim Pesan',
    contact_alt_text: 'Atau hubungi langsung melalui:',

    // Footer
    footer_copyright: '© 2026 Robby Hidayat. All rights reserved.',

    // Navigation (side dots)
    nav_about: 'Tentang',
    nav_skills: 'Keahlian',
    nav_projects: 'Proyek',
    nav_certs: 'Sertifikasi',
    nav_exp: 'Pengalaman',
    nav_contact: 'Kontak',

    // Language
    lang_switch: 'ID',
    lang_label: 'Bahasa Indonesia',
  },

  en: {
    // Preloader
    preloader_text: 'Robby Hidayat',

    // Hero
    hero_greeting: 'Hello, I\'m',
    hero_name: 'Robby Hidayat',
    hero_tagline: 'A Software Developer who builds systems with freedom & precision.',
    hero_cta_projects: 'View Projects',
    hero_cta_contact: 'Contact Me',
    hero_role: 'Software Developer',
    hero_scroll: 'Scroll down',

    // About
    about_label: 'ABOUT ME',
    about_title: 'About Me',
    about_p1: 'I\'m Robby Hidayat, a graduate of <strong>Computer Science at Lampung University</strong> with a GPA of <strong>3.74/4.00</strong>. Currently working as a <strong>Staff Programmer</strong> at PT. Lautan Teduh Interniaga, IT Division.',
    about_p2: 'Experienced in building web applications using <strong>Laravel, React, PHP, MySQL</strong>, and <strong>JavaScript</strong>. Comfortable working from requirements analysis to deployment.',
    about_p3: 'Beyond coding, I led <strong>HIMAKOM Lampung University</strong> as General Chairman with 142+ active members — an experience that forged my leadership, communication, and project management skills.',

    // Skills
    skills_label: 'SKILLS',
    skills_title: 'What I Know',
    skills_languages: 'Programming Languages',
    skills_frameworks: 'Frameworks & Tools',
    skills_soft: 'Soft Skills',

    // Soft skill items
    soft_leadership: 'Leadership',
    soft_problem: 'Problem Solving',
    soft_project: 'Project Management',
    soft_communication: 'Communication',
    soft_teaching: 'Teaching',

    // Projects
    projects_label: 'PROJECTS',
    projects_title: 'Projects',

    // Certifications
    certs_label: 'CERTIFICATIONS',
    certs_title: 'Certifications & Training',

    // Experience & Organization
    exp_label: 'EXPERIENCE',
    exp_title: 'Experience',
    exp_col_title: 'Work Experience',
    org_col_title: 'Organizations',

    // Boarding Pass
    bp_airline: 'BYRON AIRWAYS',
    bp_flight: 'DEV-2026',
    bp_gate: 'SKY-01',
    bp_seat: '01A',
    bp_class: 'FIRST CLASS',
    bp_passenger: 'ROBBY HIDAYAT',
    bp_destination: 'FULLSTACK & AI',
    bp_status: 'BOARDED',

    // Contact
    contact_label: 'CONTACT',
    contact_title: 'Let\'s Connect',
    contact_name: 'Name',
    contact_email: 'Email',
    contact_message: 'Message',
    contact_name_placeholder: 'Full name',
    contact_email_placeholder: 'email@example.com',
    contact_message_placeholder: 'Write your message here...',
    contact_submit: 'Send Message',
    contact_alt_text: 'Or reach out directly via:',

    // Footer
    footer_copyright: '© 2026 Robby Hidayat. All rights reserved.',

    // Navigation (side dots)
    nav_about: 'About',
    nav_skills: 'Skills',
    nav_projects: 'Projects',
    nav_certs: 'Certifications',
    nav_exp: 'Experience',
    nav_contact: 'Contact',

    // Language
    lang_switch: 'EN',
    lang_label: 'English',
  }
};
